...
class OrderService {
  async getOrders(filters?: Partial<OrderFilters>): Promise<OrderCollectionResponse> {
  ...
  async updateOrder(id: string, payload: UpdateOrderRequest): Promise<OrderResponse> {
    const response = await api.put<OrderResponse>(`/orders/${id}`, payload)
  ...
  async changeStatus(id: string, payload: { status: string }): Promise<OrderResponse> {
    const response = await api.patch<OrderResponse>(`/orders/${id}/status`, payload)
