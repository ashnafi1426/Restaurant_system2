<template>
  <div
    class="premium-hero-banner relative h-72 sm:h-80 md:h-96 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl"
  >
    ...
    <div
      class="absolute top-0 left-0 right-0 h-20 sm:h-32 bg-gradient-to-b from-black/60 to-transparent"
    ></div>
    ...
    <div class="absolute inset-0 flex items-center">
      <div class="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12 h-full flex items-center">
        ...
        <div
          class="w-full md:w-3/5 lg:w-1/2 pr-0 md:pr-6 lg:pr-8 flex flex-col justify-center z-10"
        >
          <!-- Small Subtitle/Tag -->
          <div class="flex items-center gap-2 mb-3 sm:mb-4 md:mb-6">
            <div
              class="w-8 sm:w-10 md:w-12 h-1 bg-gradient-to-r from-amber-400 to-amber-500 rounded-full"
            ></div>
            <span
              class="text-[10px] sm:text-xs md:text-sm font-semibold text-amber-300 tracking-widest uppercase"
            >
          ...
          <h1
            class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-serif font-bold text-white mb-2 sm:mb-3 md:mb-4 lg:mb-6 leading-tight"
          >
          ...
          <!-- Description -->
          <p
            class="text-xs sm:text-sm md:text-base text-gray-200 font-light mb-4 sm:mb-5 md:mb-6 lg:mb-8 max-w-md leading-relaxed"
          >
            {{ description }}
          </p>
          ...
          <div class="flex flex-wrap gap-2 sm:gap-3 md:gap-4">
            ...
            >
              <!-- Glow effect -->
              <span
                class="absolute inset-0 rounded-full bg-amber-400 opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-300"
              ></span>

              <!-- Content -->
              <span class="relative">{{ ctaText || 'Explore Menu' }}</span>
              <svg
                class="relative w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform duration-300"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <!-- Features Row -->
          <div
            class="grid grid-cols-3 gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 mt-4 sm:mt-5 md:mt-6 lg:mt-8 max-w-sm sm:max-w-md"
          >
            <div class="flex items-center gap-1 sm:gap-1.5 md:gap-2">
              <svg
                class="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-amber-400 flex-shrink-0"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
              ...
              <span class="text-[10px] xs:text-xs sm:text-sm text-gray-300 font-medium truncate"
                >Premium</span
              >
            </div>
            ...
            <div class="flex items-center gap-1 sm:gap-1.5 md:gap-2">
              ...
              >
                <path
                  d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zM7 10h2v7H7zm4-3h2v10h-2zm4-3h2v13h-2z"
                ></path>
              </svg>
              <span class="text-[10px] xs:text-xs sm:text-sm text-gray-300 font-medium truncate"
                >Fresh</span
        ...
        <div class="hidden md:flex w-2/5 lg:w-1/2 h-full items-center justify-end relative">
          ...
          <div
            class="absolute top-10 right-10 w-32 h-32 bg-amber-400/10 rounded-full blur-3xl"
          ></div>
          <div
            class="absolute bottom-10 right-20 w-24 h-24 bg-amber-500/5 rounded-full blur-2xl"
          ></div>
          <div
            class="absolute inset-0 bg-gradient-to-l from-transparent via-transparent to-black/30"
          ></div>
        </div>
      </div>
    </div>
    ...
    <div
      class="absolute bottom-0 left-0 right-0 h-0.5 sm:h-1 bg-gradient-to-r from-amber-400 via-amber-500 to-transparent"
    ></div>

    <!-- Corner accents -->
    <div
      class="absolute top-0 right-0 w-32 h-32 sm:w-40 sm:h-40 bg-gradient-to-br from-amber-500/20 to-transparent blur-3xl"
    ></div>
    <div
      class="absolute bottom-0 left-0 w-24 h-24 sm:w-32 sm:h-32 bg-gradient-to-tr from-amber-400/10 to-transparent blur-2xl"
    ></div>
  </div>
</template>
...
const props = withDefaults(defineProps<Props>(), {
  ...
  subHeading: 'LUXURY DINING',
  description:
    'Indulge in our meticulously curated menu, crafted by award-winning chefs using only the finest, freshest ingredients. Experience culinary excellence delivered directly to the comfort of your room.',
