...
<template>
  <div class="bg-white rounded-lg border border-gray-200 px-4 sm:px-6 md:px-8 lg:px-10 py-4 sm:py-5 md:py-6 lg:py-8">
    <h3 class="text-base sm:text-lg md:text-xl lg:text-2xl font-semibold text-gray-900 mb-3 sm:mb-4 md:mb-5">MAINTENANCE ALERTS</h3>
    ...
    <div v-else class="space-y-2 sm:space-y-3 md:space-y-4">
      <div
        ...
        :key="alert.id"
        :class="[getSeverityColor(alert.severity), 'px-3 sm:px-4 md:px-5 py-3 sm:py-4 rounded-lg border']"
      >
        <div class="flex gap-2 sm:gap-3 md:gap-4">
          ...
          <div class="flex-1 min-w-0">
            <h4 :class="[getSeverityBadgeColor(alert.severity), 'text-xs sm:text-sm md:text-base font-semibold']">
              {{ alert.title }}
            </h4>
