...
<template>
  <div class="space-y-4 sm:space-y-5 md:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3">
      ...
      <button class="px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 bg-blue-50 text-blue-600 font-bold text-xs sm:text-sm rounded-lg hover:bg-blue-100 transition whitespace-nowrap">
        FILTER: ALL ORDERS
      </button>
    </div>
    ...
    <div class="grid grid-cols-1 gap-2 sm:gap-3 md:gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="rounded-lg bg-white shadow-md overflow-hidden">
        ...
        <div class="bg-amber-50 border-b-4 border-amber-400 px-3 sm:px-4 py-2 sm:py-3">
          <h3 class="text-xs sm:text-sm font-bold uppercase tracking-widest text-amber-900">Pending</h3>
        </div>

        <!-- Orders List -->
        <div class="p-2 sm:p-3 space-y-2 sm:space-y-3 max-h-[300px] sm:max-h-[400px] md:max-h-[600px] overflow-y-auto">
          <div
          ...
          >
            <!-- Priority Badge -->
            <div v-if="order.notes" class="inline-block bg-red-100 text-red-700 text-xs font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded mb-1 sm:mb-2">
              ⚠️ <span class="hidden sm:inline">PRIORITY - ROOM</span><span class="sm:hidden">PRIORITY</span>
            </div>
            ...
            <!-- Chef Special Badge -->
            <div v-if="order.notes" class="inline-block ml-1 sm:ml-2 bg-amber-100 text-amber-700 text-xs font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
              CHEF <span class="hidden sm:inline">SPECIAL</span>
            </div>

            <!-- Time -->
            <p class="text-xs sm:text-sm font-bold text-amber-900 mt-1 sm:mt-2">{{ getTimeRemaining(order.order_time) }}</p>

            <!-- Items -->
            <div class="mt-2 sm:mt-3 space-y-0.5 sm:space-y-1 text-xs text-amber-800">
              <p v-for="(item, idx) in (order.items || []).slice(0, 3)" :key="idx" class="font-medium truncate">
                {{ item.quantity }}x {{ item.name }}
                <span v-if="item.notes" class="text-amber-600 hidden sm:inline">({{ item.notes }})</span>
              </p>
          ...
          <div v-if="!pendingOrders?.length" class="text-center py-6 sm:py-8 md:py-12 text-amber-400">
      ...
      <div class="rounded-lg bg-white shadow-md overflow-hidden">
        ...
        <div class="bg-blue-50 border-b-4 border-blue-400 px-3 sm:px-4 py-2 sm:py-3">
          <h3 class="text-xs sm:text-sm font-bold uppercase tracking-widest text-blue-900">Preparing</h3>
        </div>
        ...
        <div class="p-2 sm:p-3 space-y-2 sm:space-y-3 max-h-[300px] sm:max-h-[400px] md:max-h-[600px] overflow-y-auto">
          ...
          >
            ...
            <div class="text-xs font-bold text-blue-600 mb-1 sm:mb-2 truncate">
              TABLE {{ order.room?.room_number || '?' }} - <span class="hidden sm:inline">MAIN HALL</span>
            </div>

            <!-- Time -->
            <p class="text-xs sm:text-sm font-bold text-blue-900 mt-1">{{ getTimeRemaining(order.order_time) }}</p>
          ...
          <div v-if="!preparingOrders?.length" class="text-center py-6 sm:py-8 md:py-12 text-blue-400">
      ...
      <div class="rounded-lg bg-white shadow-md overflow-hidden">
        ...
        <div class="bg-green-50 border-b-4 border-green-400 px-3 sm:px-4 py-2 sm:py-3">
          <h3 class="text-xs sm:text-sm font-bold uppercase tracking-widest text-green-900">Ready</h3>
        </div>
        ...
        <div class="p-2 sm:p-3 space-y-2 sm:space-y-3 max-h-[300px] sm:max-h-[400px] md:max-h-[600px] overflow-y-auto">
          ...
          >
            <!-- Status Badge -->
            <span class="inline-block bg-green-200 text-green-700 text-xs font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
              ✓ <span class="hidden sm:inline">READY</span>
            </span>

            <!-- Room Info -->
            <div class="text-xs font-bold text-green-600 mt-1 sm:mt-2 mb-1 truncate">
              TABLE {{ order.room?.room_number || '?' }} - <span class="hidden sm:inline">READY</span>
            </div>

            <!-- Time -->
            <p class="text-xs sm:text-sm font-bold text-green-900 mt-1">{{ getTimeRemaining(order.order_time) }}</p>
      ...
      <div class="rounded-lg bg-white shadow-md overflow-hidden">
        ...
        <div class="bg-slate-50 border-b-4 border-slate-300 px-3 sm:px-4 py-2 sm:py-3">
          <h3 class="text-xs sm:text-sm font-bold uppercase tracking-widest text-slate-900">Served</h3>
        </div>
        ...
        <div class="p-2 sm:p-3 space-y-2 sm:space-y-3 max-h-[300px] sm:max-h-[400px] md:max-h-[600px] overflow-y-auto">
          ...
          >
            <!-- Status -->
            <span class="text-xs font-bold text-slate-600">✓ <span class="hidden sm:inline">Order</span> completed</span>
          ...
          <div v-if="!completedOrders?.length" class="text-center py-6 sm:py-8 md:py-12 text-slate-400">
