...
<template>
  <div class="border-b border-slate-200 bg-white px-4 sm:px-6 md:px-8 py-4 sm:py-5 md:py-6 shadow-sm">
    <div class="flex flex-col items-start justify-between gap-3 sm:gap-4 md:flex-row md:items-center">
      <!-- Left Section -->
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2 sm:gap-3">
          <div class="flex h-10 sm:h-12 w-10 sm:w-12 items-center justify-center rounded-xl bg-gradient-to-br from-teal-500 to-teal-600 text-lg sm:text-2xl shadow-md flex-shrink-0">
            👨‍🍳
          </div>
          <div class="min-w-0">
            <h1 class="text-lg sm:text-xl md:text-2xl font-bold text-slate-900 truncate">Kitchen Command</h1>
            <p class="mt-0.5 text-xs sm:text-sm text-slate-500 hidden sm:block truncate">Real-time order management for Main Restaurant & Room Service</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-end gap-2 sm:gap-3 w-full md:w-auto">
        ...
        <div class="relative hidden lg:block w-full md:w-auto">
          ...
          />
          <svg class="absolute right-3 top-1/2 h-3 w-3 sm:h-4 sm:w-4 -translate-y-1/2 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button class="relative rounded-lg p-1.5 sm:p-2 hover:bg-slate-100 flex-shrink-0">
          <svg class="h-5 w-5 sm:h-6 sm:w-6 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 1118 14.158V11a6 6 0 00-9-5.532V5a6 6 0 006 6h.666v4.3a1.997 1.997 0 01-.666 1.4M9 17h6" />
          </svg>
          <span class="absolute top-0.5 right-0.5 h-1.5 sm:h-2 w-1.5 sm:w-2 rounded-full bg-red-500"></span>
        </button>
        ...
        <!-- User Profile -->
        <div class="hidden sm:flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2 sm:px-3 py-1.5 sm:py-2 flex-shrink-0">
          <div class="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 flex-shrink-0"></div>
        ...
        <div class="flex gap-1 sm:gap-2 w-full sm:w-auto">
          ...
          >
            <span>+</span> <span class="hidden sm:inline">NEW TICKET</span><span class="sm:hidden">NEW</span>
          </button>
          <button
