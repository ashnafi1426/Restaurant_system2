  ...
  >
    <div class="flex flex-col sm:flex-row items-start justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
      <div class="flex items-center gap-2 sm:gap-3 min-w-0">
        <div class="flex items-center gap-1 sm:gap-2 flex-wrap">
          <span v-if="order.priority" class="text-xs font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-1.5 sm:px-2 py-0.5 rounded whitespace-nowrap">
            PRIORITY
          </span>
          <span class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 whitespace-nowrap">
            ROOM {{ order.room }}
          </span>
          <span v-if="order.customer" class="text-xs text-gray-400 hidden sm:inline truncate">• {{ order.customer }}</span>
        </div>
      </div>
      <div class="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        <span class="text-xs font-medium text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-600 px-1.5 sm:px-2 py-0.5 rounded-full shadow-sm whitespace-nowrap">
          ⏱ {{ order.time }}
        </span>
        <button @click="$emit('view-details', order)" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition flex-shrink-0">
    ...
    <div class="space-y-1 sm:space-y-2">
      <div v-for="(item, index) in order.items" :key="index" class="flex items-start gap-2">
        <span class="text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-100 min-w-[24px] flex-shrink-0">
          {{ item.quantity }}x
        </span>
        <div class="flex-1 min-w-0">
          <p class="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{{ item.name }}</p>
          <p v-if="item.note" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">{{ item.note }}</p>
        </div>
      </div>
    </div>

    <!-- Actions -->
    <div class="flex items-center gap-1 sm:gap-2 mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-200 dark:border-gray-600 flex-wrap">
      ...
      <button class="p-1.5 sm:p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition flex-shrink-0">
