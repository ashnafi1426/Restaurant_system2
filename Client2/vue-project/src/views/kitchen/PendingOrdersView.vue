...
const { pendingOrders, statistics, actionLoading } = storeToRefs(kitchenStore)
    ...
    <div class="space-y-6">
      ...
      <div v-if="pendingOrders?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="order.id"
          class="bg-white rounded-lg border-l-4 border-amber-400 p-6 shadow-md hover:shadow-lg transition"
        >
          <!-- Room Info -->
          ...
          <!-- Special Notes -->
          <div v-if="order.notes" class="mb-4 p-3 bg-red-50 border border-red-200 rounded">
            <p class="text-xs text-red-700">⚠️ {{ order.notes }}</p>
          </div>
          ...
          <button
            ...
            class="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-bold py-2 rounded-lg transition"
          >
            {{ actionLoading !== null ? 'Starting...' : '👨‍🍳 START PREPARING' }}
          </button>
        </div>
      </div>
