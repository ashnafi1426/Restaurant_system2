...
function calculateStatistics(): void {
  ...
  statistics.value.pending_orders =
    orders.value.filter(
      order => order.status === 'pending'
    ).length

  statistics.value.preparing_orders =
    orders.value.filter(
      order => order.status === 'preparing'
    ).length

  statistics.value.ready_orders =
    orders.value.filter(
      order => order.status === 'ready'
    ).length

  statistics.value.served_orders =
    orders.value.filter(
      order => order.status === 'served'
    ).length

  statistics.value.cancelled_orders =
    orders.value.filter(
      order => order.status === 'cancelled'
    ).length

  statistics.value.total_revenue =
    orders.value.reduce(
